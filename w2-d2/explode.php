<?php
function myExplode($delim,$myString){ //declaring function/user declared
	...
	if($myString[$currentindex]==" "){//if evaluates whether the character is a space or not
	...
	//the "if" executes the code if the condition is true//so myString compares to a space then it will run// array_push is a function it pushes the variables// 
		...
		$currWord = $currWord.$myString[$currentindex];//builds word one character at a time
	}
	}
